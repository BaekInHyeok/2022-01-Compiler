...
%token	ADD SUB MUL DIV REM >= <= > < == ASSGN ID NUM STMTEND START END ID2
%left ADD SUB
%left MUL DIV REM
...
stmt	: 	ID ASSGN expr STMTEND	{ $1->token = ID2; $$=MakeOPTree(ASSGN, $1, $3);}
		|	IF expr stmt			{ $1->token = ID2; $$=MakeOPTree(ASSGN, $1, $3);}
		;
...
expr	: 	expr ADD term	{ $$=MakeOPTree(ADD, $1, $3); }
		...
		|	expr REM term	{ $$=MakeOPTree(REM, $1, $3); }
		|	expr <=	term	{ $$=MakeOPTree(<
